...
import { speakers } from './shared/schema.ts';
...
const socialMediaData = JSON.parse(fs.readFileSync('social_media_updates.json', 'utf8'));
...
async function updateSpeakerSocialMedia() {
  try {
    ...
    for (const socialSpeaker of socialMediaData) {
      ...
      if (!matchedSpeaker) {
        ...
        matchedSpeaker = dbSpeakers.find(dbSpeaker => {
          ...
          return dbLastName === socialLastName && socialLastName.length > 3; // Avoid very short names
        });
      }
    ...
    console.log(`\n=== SUMMARY ===`);
    console.log(`Total social media records: ${socialMediaData.length}`);
    console.log(`Matched speakers: ${matchedCount}`);
    console.log(`Updated speakers: ${updatedCount}`);
    console.log(`Unmatched: ${socialMediaData.length - matchedCount}`);
    
    // Save update log
    fs.writeFileSync('social_media_update_log.json', JSON.stringify(updateLog, null, 2));
    console.log(`\nUpdate log saved to social_media_update_log.json`);
